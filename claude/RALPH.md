# RALPH.md — opt-in operating rules for lean coding + Ralph Loop

> Reference this file with `@RALPH.md` from a CLAUDE.md or paste as a one-liner when running Ralph Loop or any session where these rules should bias behavior. Not auto-loaded.

Promoted from `claude-build/CLAUDE.md` v1.2 (2026-05-04) on 2026-05-09 during the cc-full ↔ cc-build merge.
...
6. Per-iteration `lessons.md` MUST contain `## What worked`, `## What failed`, `## Off-limits attempts`, `## Suggested rule for RALPH.md`. Append to `<project>/.ralph/notes.md` at iteration end.
...
## Vibe coding rules (apply in any lean coding session, with or without Ralph)
